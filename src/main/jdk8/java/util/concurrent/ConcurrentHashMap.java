 ...
 * <p>
 * 这些批量操作接受 一个并行阈值参数
 * 如果当前map长度比给定的阈值，方法是可以按顺序进行的
 * 使用一个Long最大值 抑制所有的并行性
 * 使用值*为{@code 1}，通过划分成*足够的子任务来充分利用用于所有并行*计算的{@link * ForkJoinPool#commonPool()}，从而获得最大的并行度
 * <p>
 * 使用1结果在 最大并行 通过一部分进入足够的子任务去完全利用，以至于被用在所有的并行计算
 * 平常，你需要初始化选择一个在他们极端的值里面，然后测量性能使用 中间的值，权衡开销和吞吐量
 *
 *
 *
 ...
 * <p>
 * <p>
 * 大容量批量操作遵循ConcurrentHashMap的并发配置，
 * <p>
 * 从{@code get(key)}和相关的访问方法返回的任何非空结果与相关的插入或*更新具有* happens-before
 * 任何不为空的结果返回从get方法并且相关的方法 与插入或更新的操作有 happens-before 关系
 * 这个结果对于任何批量操作的的影响是 作为关系每一个元素的组成(处理必须的 原子类的遵循map 作为除了任何已知的)
 * <p>
 * 任何批量操作结果都反映了这些每个元素的组成（对于map整体而言不一定是原子的，除非它以某种方式被知道是静态的）
 * 相反的，由于key和value在map中都不能为null,null服务作为可靠的原子指示器 在当前缺乏任何结果
 * 为了维持这个配置，null 服务作为一个基本的隐式对于所有没有标量的减少操作
 * 对于double,long和int 版本，当与其他任何值结合时，应该返回其他值（更正式的说，它应该是进行过简约的身份元素）
 * <p>
 * 常见的一些配置的配置，填充一个sum 和基础的0或者一个饿比 MAX_VALUE 小的数据
 *
 *
 ...
 * reduce operations.
 * <p>
 * 查询和转换方法作为参数提供，必须类似返回null去表明缺乏任何结果，就是说它是没有被使用的
 * 另一方面 映射减少，这也能够 转换服务作为过滤，返回null 或则会
 * 如果元素没有被结合，在减少映射的情况下，返回null是可以被应用在作为过滤上，或者在源语情况下，返回标识基
 * 你可以创建一个复合转换并且过滤通过你们自己的 null 代表没有任何东西创建 规则，在使用他们进行查询或者减少操作
 *
 ...
 * <p>
 * 方法接受 and/or 返回实体参数维护 键值对关联
 * 他们也许是有用的例如当发现一个key有一个最大的值。
 * 计划使用 Entry 参数能够被支持应用
 *
 ...
 * not occurred.
 * <p>
 * 大批量操作可能突然完成，引发应用程序中遇到函数异常。
 * 在处理这一类异常时，其他并发执行的函数也可能会抛出异常，如果第一个异常没有发生，也会抛出异常
 *
 ...
 * 与顺序形式相比，并发形式加速是常见的，但不能保证。
 * 并行操作涉及短暂的方法在一些小的集合也许会执行慢比 顺序执行，如果它潜在的工作在并行底层 是昂贵的比完成它自己
 * 类似的，并行也许没有领导其他多的实际，如果所有的处理器都在忙其他无关的任务
 *
 *
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
     ...
     * 无论哪一个先实现。这个case不是最快的，但是最大化总预期吞吐量
     *
     *
     *
     ...
     * DEFAULT_CAPACITY.
     * <p>
     * 每个转移步骤的最小重组数。范围*细分，以允许多个大小调整器线程。这个值*用作下界，以避免大小调整器遇到*过多的内存争用。至少为* DEFAULT_CAPACITY。
     */
     ...
     * 基于计数器的值，主要被应用在没有争用，但是仍然回退根据table初始化，通过CAS 进行初始化
     */
     ...
     * <p>
     * 初始化和调整控制，当是负数的时候，table正在进行初始化或者调整：-1是默认值，否则就是 -（1+活跃的调整线程数量）
     * 另外，当table是空的，持有这个初始化table长度用于创建，或者默认为0
     * 初始化完毕，持有下一个元素数量值当调整table时
     * 默认是0，当构造函数携带参数时，会有值信息
     * <p>
     * -1：容器正在初始化 -N 有N-1个线程正在进行扩容
     * 0：容器未初始化
     * >0 初始化或者下一次进行扩容的大小
     */
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if ((eh = e.hash) == h) {
                if ((ek = e.key) == key || (ek != null && key.equals(ek)))
                    return e.val;
            } else if (eh < 0) //红黑树或者转移节点，使用对应的方法获取
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            //当前桶元素的类型
            int fh;
            //table是空的，进行初始化 TODO 初始化过程
            if (tab == null || (n = tab.length) == 0)
                tab = initTable();
                //如果当前索引位置没有值，直接进行创建
                // 获取桶的第一个元素  TODO 为什么不使用 tab[i = (n - 1) & hash] ?
            ...
            } else if ((fh = f.hash) == MOVED)//桶首个元素是转移节点，表示该节点正在进行扩容 会一直等待扩容完成
                // TODO 协助进行转换
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        //红黑树
                            ...
                            Node<K, V> p;
                            binCount = 2;//由于后续对于 binCount >= 8 的链表进行转换树结构处理，设置为2时，不会进行任何操作
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0)//小于0说明正在进行初始化,释放当前线程调度权，重新发起锁竞争
                Thread.yield(); // lost initialization race; just spin
                // 使用CAS 锁住节点  sc是原值，-1是期望值
                //保证只有一个线程在初始化， -1 代表当前只能有一个线程
                ...
                try {
                    ...
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        //table指向新创建的节点数组
                        table = tab = nt;
                        // TODO 这个又是在做什么妖？ 为什么不直接表示 sc = 3/4 n ?
                        // 表示下一次需要扩容时的大小，指定负荷因子时，构造函数 sizeCtl 已经计算好了
                        sc = n - (n >>> 2);
                    }
                } finally {
                    sizeCtl = sc;
     ...
     *
     * 增加数量，如果table太小，并且没有进行调整，启动转换，如果已经调整了，帮助程序
     * 进行转换如果工作允许，
     * 重新检查占用在转换之后去查看 如果另外一个调整已经完毕需要，因为调整在添加元素之后
     *
    ...
    private final void addCount(long x, int check) {
        ...
        if (check >= 0) {
            Node<K, V>[] tab, nt;
            int n, sc;
            //如果map.size()  大于 sizeCtl 说明需要进行过扩容
            // rable 不是空，且table长度达到 1<<30 可以进行扩容
            while (s >= (long) (sc = sizeCtl) && (tab = table) != null &&
                    (n = tab.length) < MAXIMUM_CAPACITY) {
                int rs = resizeStamp(n);
                if (sc < 0) {
                    // 如果sc的低16位不等于标识符（sizeCtl 发生变化）
                    // 如果 sc == 标识符+1，扩容结束，不再有线程进行扩容
                    // 如果 sc =  标识符+65535，说明帮助线程数量达到最大
                    // 如果nextTable == null , 说明扩容结束了
                    /// 如果 transferIndex <=0 转移状态发生变化
    ...
    final Node<K, V>[] helpTransfer(Node<K, V>[] tab, Node<K, V> f) {
        ...
        Node<K, V>[] nextTab;
        int sc;
        //属于转移节点类型 并且要转移的目标table已经存在
        if (tab != null && (f instanceof ForwardingNode) &&
                (nextTab = ((ForwardingNode<K, V>) f).nextTable) != null) {
            int rs = resizeStamp(tab.length);
            // TODO 多线程是否会有问题？ table == tab
            // nextTab 在上一步已经声明了 (nextTab = ((ForwardingNode<K, V>) f).nextTable
            //如果while循环中 nextTab != nextTable,说明此刻 nextTab 指向的 与 map集合中的 nextTable 不一致
            // tab 是传入进来的 table指向，如果tab指向与mao中table指向不一致，说明有多线程进行修改，就不用再处理了
            // sc = sizeCtl) < 0 说明正在处于扩展状态
            while (nextTab == nextTable && table == tab &&
                ...
                if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
                    ...
                    break;
                // 使用CAS获取锁，一次只能有一个操作成功 TODO 但是可以有多个线程同时获取不同的锁进行操作，这个如何解？
                if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1)) {
     ...
     * <p>
     * 移动或者复制桶上的每个节点到新的table
     * 扩容的时候会一直进行遍历，直到复制完所有节点，每处理一个节点都会在链表表头设置一个 fwd 节点，这样其他线程就会跳过他
     * 复制后在新数组中的链表不是绝对反序的
     */
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        //旧数组长度
        int n = tab.length;
        //步长：表示好一个线程处理的数组长度，控制CPU的使用
        //每个CPU最少处理16个长度的数组元素，如果一个数组的长度只有16，那么只有一个线程会对其进行扩容的复制和移动操作
        int stride;
        // 控制不要占用太多的CPU,最小步长为16   为什么是原table长度/8/CPU核数
        ...
        //创建一个移动节点，表示原数组中位置i处的节点完成迁移后，就会在i位置设置一个fwd节点告诉其他线程这个位置已经被处理过了
        ForwardingNode<K, V> fwd = new ForwardingNode<K, V>(nextTab);
        //是否需要继续推进，为true说明需要推进一个下标i--
        //每次将一个桶中的元素转移完成之后，会重新置为 true
        boolean advance = true;
        //完成之前重新扫描一边数组
        boolean finishing = false; // to ensure sweep before committing nextTab
        //无限自旋，i的值会从原数组的最大值开始慢慢递减到0
        //i 当前要处理的槽位序号
        //bound 要处理的槽位边界，先处理槽位15的节点
        for (int i = 0, bound = 0; ; ) {
            //使用过CAS不断尝试为当前线程分配任务，直到分配成功或者队列已经被全部分配完毕
            //如果当前线程已经被分配过桶，那么会通过--i,指向下一个待处理的桶，然后退出循环
            //桶的头节点？
            Node<K, V> f;
            //头节点hash值
            int fh;
            while (advance) {
                ...
                //下一界限？
                int nextBound;
                //如果结束了 或者  --i >= bound 就可以进行退出了？ TODO 为什么是--i?
                // --i 表示下一个待处理的桶，如果 >= bound,说明当前线程已经被分配过了桶区域
                // 最开始为i=0， -1 >= 0 必定不成立
                // 通过第一次获取处理的步长，确定最小位置角标，如果小于，说明已经处理完成，不再进行往前推进
                if (--i >= bound || finishing)
                    advance = false;
                    //表示所有的桶都已经被分配完毕，给 nextIndex 赋予初始值 16
                    // transferIndex 是变动的，通过CAS设置 U.compareAndSwapInt(this, TRANSFERINDEX, nextIndex,nextBound = (nextIndex > stride ? nextIndex - stride : 0))
                    // 值越来越小
                    //transferIndex 代表下一次处理的开始位置
                    // (nextIndex = transferIndex) <= 0 说明下一次要处理的角标位置为0，往前推进，但是角标最小就为0，因此不再推进
                else if ((nextIndex = transferIndex) <= 0) {//该判断什么时候可以进入？
                    i = -1;
                    advance = false;
                    //每次减少i的值
                    //通过CAS来修改 TRANSFERINDEX ，为当前线程分配任务，区间nextBound,nextIndex ->(0,15)
                } else if (U.compareAndSwapInt
                        (this, TRANSFERINDEX, nextIndex,
                                nextBound = (nextIndex > stride ?
                                        nextIndex - stride : 0))) {
                    //当 nextIndex > 步长时，说明这个线程处理不完数据，剩下的留给下一个线程进行处理，记录一下下一次元素处理的开始位置
                    //如果能够处理完毕，则下一次处理的开始位置为0，不需要再次进行处理
                    // transferIndex 维护在内部变量中，如果其他线程操作同一个map进行协助扩容时，当无元素需要再次进行处理时，则直接退出该判断
                    //本次处理的最低边界角标
                    bound = nextBound;
                    // 获取任务之后，从后往前进行处理
                    i = nextIndex - 1;
                    //表示当前线程已经获取到了对应的处理区间，不需要再次进行推进获取
                    advance = false;
                }
            }
            // i < 0 说明已经遍历完旧数组，即当前线程已经处理完所有负责的桶
            // i >= n 当前已经处理的角标位置大于旧容器的长度，说明容器发生改变？ TODO
            // i + n >  nextn 当前处理元素位置 + 旧容器长度 >= 新容器长度，说明容器发生改变 ？ TODO
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                // SIZECTL 在迁移前会被设置为 (rs << RESIZE_STAMP_SHIFT) + 2  TODO 为什么？
                // 使用 CAS操作对 SIZECTL 的低16位进行减1，说明已经完成自己的任务
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {
                    //第一个线程扩容之前，执行transfer方法之前，会设置 SIZECTL = (resizeStamp(n) << RESIZE_STAMP_SHIFT) + 2)
                    //后续帮其扩容的线程，执行 transfer 方法之前，会设置 izeCtl = sizeCtl+1
                    //每一个退出 transfer 的方法的线程，退出之前都会设置 sizeCtl = sizeCtl-1
                    //那么最后一个线程退出时，必然有 sc == (resizeStamp(n) << RESIZE_STAMP_SHIFT) + 2)，即 (sc - 2) == resizeStamp(n) << RESIZE_STAMP_SHIFT


                    //如果 sc - 2 不等于标识符左移 16 位 ，如果他们相等了，说明没有线程在帮助其进行扩容，即扩容结束
                ...
                //如果 i 位置是空的，没有任何节点，放置初始化的转移节点
                advance = casTabAt(tab, i, null, fwd);
            else if ((fh = f.hash) == MOVED)
                // 说明已经完成了迁移，即有线程完成了这个节点，继续推进下一个节点
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K, V> p = f.next; p != null; p = p.next) {
                                //链表下面的节点和上面的节点 hash 计算不一致
                                int b = p.hash & n;
                                //找出最后一个 hash值与其他不一样的数据
                                if (b != runBit) {
                                    runBit = b;
                                    lastRun = p;
                                }
                            }
                            // runBit = 0 时，最后一个元素是低位链表表头，高位链表表头为null
                            if (runBit == 0) {
                                ln = lastRun;
                                hn = null;
                            } else {
                                //最后一个元素是高位表头，低位表头元素为null
                                hn = lastRun;
                                ln = null;
                            }
                            for (Node<K, V> p = f; p != lastRun; p = p.next) {
                                ...
                                K pk = p.key;
                                V pv = p.val;
                                //组装低位链表  如果最开始 ln为null,则链表是和原链表顺序保持一致，新链表头元素为符合条件的倒数第一个元素
                                //如果最开始 ln 不为空，则链表的第一个元素是尾部元素，剩余元素顺序与原链表保持一致，新链表的头元素是符合条件的倒数第二个元素
                                if ((ph & n) == 0)
                            ...
                            for (Node<K, V> e = t.first; e != null; e = e.next) {
                                ...
                                if ((h & n) == 0) {
                                    // 当前节点设置前一个节点为高位链表头节点为空，设置当前节点为头节点
                                    if ((p.prev = loTail) == null)
                                        lo = p;
                                    else
                                        //前一个元素设置成功，设置前一个元素的下一个元素为新建元素
                                        loTail.next = p;
                                    //当前新建元素为低链表尾节点
                                    loTail = p;
                                    ++lc;
            ...
            for (TreeNode<K, V> p = root; ; ) {
                ...
                if (p == null) {
                    ...
                    return p;
                        ...
                        (kc = comparableClassFor(k)) == null) ||
                        (dir = compareComparables(kc, k, pk)) == 0) {
                    // 如果找不到元素，说明整棵树上都没有，下一次不需要再次执行了
                    if (!searched) {
                        TreeNode<K, V> q, ch;
                        searched = true;
                        // 上述已经表明当前节点不是要查询的节点，从左右两个子树查询有没有要找寻的节点，找到节点，返回
                        if (((ch = p.left) != null &&
                    ...
                    //看目标节点位置在当前节点位置左侧还是右侧
                    dir = tieBreakOrder(k, pk);
                }

                //说明没有找到元素
                TreeNode<K, V> xp = p;
                //根据目标元素在当前元素的左边还是右边，如果相对应的位置没有元素，则执行插入操作
                if ((p = (dir <= 0) ? p.left : p.right) == null) {
